...
p0 = np.fromfile('/data/jahn/ECCO/quota1x1/dic_mmolm3_jan2004.bin', '>f4').reshape((-1,160,360))
p1 = np.ma.zeros(p0.shape[:-2] + f1.shape[::-1], p0.dtype)
...
p2 = p1.copy()
for k in range(1, p0.shape[0]):
    msk = np.isnan(p2[k])
    p2[k][msk] = p2[k-1][msk]


g2 = create_grid_3d((128,64), -rf0)
g3 = create_grid_3d((128,64), -rf1)

m2 = g2.add_item(ESMF.GridItem.MASK)
m3 = g3.add_item(ESMF.GridItem.MASK)
...
f2 = ESMF.Field(g2, 'src')
f3 = ESMF.Field(g3, 'dst')

ff2 = ESMF.Field(g2, 'srcfrac')
ff3 = ESMF.Field(g3, 'dstfrac')

regridVert = ESMF.Regrid(f2, f3,
                         src_mask_values=np.array([1], dtype=np.int32),
                         dst_mask_values=np.array([1], dtype=np.int32),
                         regrid_method=ESMF.RegridMethod.CONSERVE,
                         unmapped_action=ESMF.UnmappedAction.IGNORE,
...
p3 = np.zeros(f3.shape[::-1], '>f4')

for fname in [
'C_bio_small.bin',
'Chl_bio_small.bin',
'dic_mmolm3_jan2004.bin',
'DOC_run1_13_10y.bin',
'DOFe_run1_13_10y.bin',
'DON_run1_13_10y.bin',
'Fe_bio_small.bin',
'lev01_nitrate_ann-3d.bin',
'N_bio_small.bin',
'NH4_run1_13_10y.bin',
'NO2_run1_13_10y.bin',
'payal_fet_ann-3d.bin',
'POC_run1_13_10y.bin',
'POFe_run1_13_10y.bin',
'PON_run1_13_10y.bin',
]:
    ...
    p3.tofile('quota28x28/{}'.format(fname))
