'''
modified version of MITgcmutils/mds able to read a single metafile for tiles data
'''
import sys
import os
import re
...
class MdsArray(np.ndarray):
    def __new__(cls, input_array, metadata=None, iterations=None):
    ...
    def __array_finalize__(self, obj):
        ...
        self.iterations = getattr(obj, 'iterations', None)
...
def parsemeta(metafile):
    ...
    for line in lines:
        ...
        if opening == '[':
            ...
            # {} contains space-separated, possibly single-quoted items
            val = line.strip().split("'")
            assert len(val) % 2 == 1
            # strip single-quoted strings
            for i in range(1, len(val)//2*2, 2):
                val[i] = val[i].rstrip()

            # split non-quoted parts of line
            for i in range(len(val)//2*2, -1, -2):
                val[i:i+1] = val[i].split()
...
def scanforfiles(fname, ext='.meta'):
    """ return list of iteration numbers for which metafiles with base fname exist """
    import glob
    allfiles = glob.glob(fname + '.' + 10*'[0-9]' + '.001.001' + ext)
    if len(allfiles) == 0:
        allfiles = glob.glob(fname + '.' + 10*'[0-9]' + ext)
        off = -5
    else:
        off = -13
...
def rdmds(fnamearg,itrs=-1,machineformat='b',rec=None,fill_value=0,
          ...
          usememmap=False,mm=False,squeeze=True,quiet=False):
    """ a     = rdmds(fname,...)
    a     = rdmds(fname,itrs,...)
    ...
    if os.path.exists(metafile):
        ...
        metafiles = []
        for i in range(n):
            v = list(map(int, l[i*7:i*7+6]))
            metafiles.append(v + [l[i*7+6]])
        del l
        dir, _ = os.path.split(fnamearg)
        deltaT = timeinterval[1]/timestep[0]
        stepT = timeinterval[1] - timeinterval[0]
        tiles = True
    else:
        warning('Single metafile not found: ' + metafile + '\n')
        tiles = False

    if itrs in [np.nan, np.inf]:
        if tiles:
            itile, jtile, i1, i2, j1, j2, pre = metafiles[0]
            datafile = os.path.join(dir, pre)
            allitrs = scanforfiles(datafile, '.data')
        else:
            allitrs = scanforfiles(fnamearg, '.data')
    ...
    if itrs is np.nan:
        # all iterations
        itrs = allitrs
        if not quiet: warning('Reading {0} time levels: '.format(len(itrs)), *itrs)
        returnits = True
        itrsislist = True
    elif itrs is np.inf:
        # last iteration
        if len(allitrs):
            if not quiet: warning('Found {0} time levels, reading iteration'.format(len(allitrs)), allitrs[-1])
        else:
            if not quiet: warning('Found 0 time levels for {}'.format(fnamearg))
        itrs = allitrs[-1:]
        returnits = True
        itrsislist = False
    else:
    ...
    for iit,it in enumerate(itrs):
        if additrs:
            itext = '.{0:010d}'.format(int(it))

        if not tiles:
            fname = fnamearg + itext
            metafiles = glob.glob(fname + 2*('.'+3*'[0-9]') + '.meta') or glob.glob(fname+'.meta')
            if len(metafiles) == 0:
                raise IOError('No files found for ' + fname + '.meta')

            if not quiet: warning(metafiles[0])

            if debug: warning('Found',len(metafiles),'metafiles for iteration',it)

        for metafile in metafiles:
            if tiles:
                itile, jtile, i1, i2, j1, j2, pre  = metafile
                datafile = os.path.join(dir, pre) + '{0}.{1:03d}.{2:03d}.data'.format(itext, itile, jtile)
                i0s = (len(gdims) - 2)*[0] + [j1 - 1, i1 - 1]
                ies = list(gdims[:-2]) + [j2, i2]
                if additrs:
                    timestep = [it]
#                    t1 = deltaT*it
#                    t0 = t1 - stepT
#                    timeinterval = [t0, t1]
    ...
    if arr is None:
        ...
        if squeeze:
            ...
            keepers = [itrsislist, allrec or np.iterable(rec)] + [np.iterable(l) for l in lev]
...
def writemeta(fname, itr, dims, i0s, ies, timestep, timeinterval, map2gl,
              dataprec='float32', fields=None, simulation=None,
              nrecords=None, missingValue=None):
    with open(fname, 'w') as f:
        ...
        f.write(" dimList = [\n " +
            ",\n ".join(fmt.format(d, s+1, e) for d,s,e in zip(dims, i0s, ies)[::-1]) +
            "\n ];\n")
        ...
        if timeinterval is not None:
            f.write(" timeInterval = [" +
                    "".join("{: 20.12E}".format(t) for t in timeinterval) +
                    " ];\n")

        if missingValue is not None:
            f.write(" missingValue = [ {: 21.14E} ];\n".format(missingValue))
...
def readmds(fnamearg,itrs=-1,machineformat='b',rec=None,fill_value=0,
            astype=None, lev=(), fld=None,
            usememmap=False,mm=False,squeeze=False,quiet=False):
    """ a     = rdmds(fname,...)
    a     = rdmds(fname,itrs,...)
    a,its,meta = rdmds(fname,...,returnmeta=True)
    ...
    if itrs is np.nan:
        ...
        itrs = scanforfiles(fnamearg)
        if len(itrs):
            if not quiet: warning('Found {0} time levels, reading'.format(len(itrs)), itrs[-1])
        else:
            if not quiet: warning('Found 0 time levels for {}'.format(fnamearg))
        itrs = itrs[-1:]
        returnits = True
    ...
    allrec = rec is None and fld is None
    keeprecdim = allrec or np.iterable(rec) or np.iterable(fld)
    reclist = aslist(rec)
    if not isinstance(lev,tuple):
        lev = (lev,)
    levs = tuple( aslist(l) for l in lev )
    ...
    if allrec:
        ...
        fldlist = meta['fldlist']
        reclist = [fldlist.index(f) for f in aslist(fld)]
    elif fld is None:
        recinds = np.ix_(reclist, *levs)
    else:
        raise "rec and fld cannot both be given"

    assert nlev+2 <= len(gdims)
    rdims = levdims + gdims[len(levdims):]
    # always include itrs and rec dimensions and squeeze later
    arr = np.empty((len(itrs),len(reclist))+rdims, astype).view(MdsArray)

    for iit,it in enumerate(itrs):
        datafile = fnamearg + '.{0:010d}.data'.format(int(it))

        if recsatonce:
            arr[iit] = np.fromfile(datafile, tp).reshape(tileshape)[recinds]
        else:
            f = open(datafile)
            for irec,recnum in enumerate(reclist):
                if recnum < 0: recnum += nrecords
                f.seek(recnum*count*size)
                arr[iit, irec] = np.fromfile(f, tp, count=count).reshape(recshape)[levinds]
            f.close()

        if timestep is not None:
            timeStepNumbers.append(it)
...
#            timeIntervals.append(timeinterval)
    ...
    # put list of iteration numbers back into metadata dictionary
    if len(timeStepNumbers):
        meta['timeStepNumber'] = timeStepNumbers

    if len(timeIntervals):
        meta['timeInterval'] = timeIntervals
    ...
    if squeeze:
        ...
        # squeeze all that came from scalar arguments
        keepers = [itrsislist, keeprecdim] + [np.iterable(l) for l in lev]
        squeezed = tuple( d for d,keep in zip(dims, keepers) if keep )
