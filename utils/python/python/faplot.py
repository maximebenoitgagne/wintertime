import numpy as np
from matplotlib.pyplot import gca, sci, draw_if_interactive
...
def backmask(x, y):
    ...
    ar = d1[0]*d2[1] - d1[1]*d2[0]
    # mask quads with area the opposite sign of the majority
    msk = (ar/np.median(ar)) < -1.0
    return msk


def pcolormeshes(*args, **kwargs):
    """
    ...
    *C* may be a Facets object or a sequence of arrays.  In the latter
    ...
    try:
        ret = axpcolormeshes(ax, *args, **kwargs)
        draw_if_interactive()
    finally:
        ax.hold(washold)
    sci(ret)
...
def axpcolormeshes(ax, *args, **kwargs):
    if not ax._hold:
        ax.cla()
    ...
    X, Y, C = args
    Ny = [x.shape[0]-1 for x in X]
    Nx = [x.shape[1]-1 for x in X]
    sizes = [(x.shape[0]-1)*(x.shape[1]-1) for x in X]
    inds = np.cumsum([0] + sizes)
    Xsizes = [(x.shape[0])*(x.shape[1]) for x in X]
...
#                print 'applying mask per facet'
                try:
                    for i in range(n):
                        C[i].mask |= msk[i]
                except AttributeError:
#                    print 'creating masked arrays'
                    C = [np.ma.MaskedArray(C[i], msk[i]) for i in range(n)]
#        print C[0].shape
    ...
    try:
        ...
        tmp = np.zeros((sum(sizes),)+C[0].shape[2:], C[0].dtype).view(C[0].__class__)
        for i in range(len(sizes)):
            tmp[inds[i]:inds[i+1]] = C[i].reshape((-1,) + C[i].shape[2:])
        C = tmp
    else:
        if C.ndim > 1:
            C = C.T
    ...
    coords = [coordsbase[Xinds[i]:Xinds[i+1]].reshape(X[i].shape+(2,)) for i in range(n)]
    for i in range(n):
        coords[i][:,:,0] = X[i]
        coords[i][:,:,1] = Y[i]
    ...
    if C.ndim > 1:
        ...
        if kwargs['edgecolors'].lower() == 'none':
            collection.set_edgecolors(C*0)
    else:
    ...
    for i in range(len(sizes)):
        if t and any(t.contains_branch_seperately(ax.transData)):
            ...
            pts = np.vstack([X[i], Y[i]]).T.astype(np.float)
...
class MultiQuadMesh(Collection):
    ...
    def __init__(self, meshWidth, meshHeight, coordinates,
        ...
        self._coordinates = [np.array(c.reshape(h+1, w+1, 2), np.float_) 
