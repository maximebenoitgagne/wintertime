...
class MNC(object):
    def __init__(self, patt):
        ...
        self.ds = [ Dataset(f) for f in files ]
        ds = self.ds[0]
        self.Nx = ds.Nx
        self.Ny = ds.Ny
        self.Nr = ds.Nr
        self.Nt = ds.variables['T'].size
    ...
    def get(self, name, s=Ellipsis):
        var0 = self.ds[0].variables[name]
        dims = tuple( self.dims[dim] for dim in var0.dimensions )
        ndims = len(dims)
        ...
        res = np.zeros(sliceshape(s, dims), var0.dtype)
