...
class FormattedArray(np.ndarray):
    def __new__(subtype, filename, fmt, header=0, section=(0,1)):
        fmt = FortranFormat(fmt)
        skip = header
        titlelen,datalen = section
        seclen = titlelen + datalen
        file = open(filename)
        header = []
        sections = []
        section = []
        ...
        for iline,line in enumerate(file):
            if iline < skip:
                header.append(line)
            elif (iline-skip) % seclen < titlelen:
                section.append(line)
                if (iline-skip) % seclen == titlelen - 1:
        ...
        obj = np.ndarray.__new__(subtype, (nrows,ncols), object)
        obj.format = fmt
        obj.seclen = datalen
        obj.header = ''.join(header)
    ...
    def __array_finalize__(self,obj):
        ...
        self.sections = getattr(obj, 'sections', {})
        self.seclen = getattr(obj, 'seclen', {})
...
%(data)s,
   ...
   header=%(header)s,
   sections=%(sections)s)"""
        return desc % {'data': str(self), 'format':repr(self.format.fields), 'header':self.header, 'sections':self.sections }
    ...
    def write(self, file):
        ...
        for i,sec in enumerate(self.sections):
            ...
            for r in range(i*self.seclen, (i+1)*self.seclen):
                fline = FortranLine(self[r], self.format)
                file.write(str(fline) + '\n')
...
if __name__ == '__main__':
    ...
    try:
        ...
        sys.exit('Usage: ' + me + ''' FORMAT FILE
        
Copies FILE to standard output, converting every line in FILE that
starts with a space according to the fortran format specifier FORMAT.
Fields in the input are assumed to be separated by whitespace.

Example: ''' + me + ' "I4,3F10.4" file')

    a = FormattedArray(fname, fmt)
    a.write(sys.stdout)
