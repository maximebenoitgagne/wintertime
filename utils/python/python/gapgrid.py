...
class Grid(object):
    ...
    def set_label_mode(self, mode):
        ...
        if mode.lower() == "none":
            ...
            for col in self.axes_column:
                # axes with no x labels
                for ax in col[:-1]:
                    _tick_only(ax, bottom_on=True, left_on=False)
...
#         return vsize, hsize
    ...
    def add_axes(self, r, c):
        ax = self.axes_row[r][c]
        self.figure.add_axes(ax)
        return ax

    def imshow(self, arrs, perc=(None, None), scale=None, mask=None, **kwargs):
        kwargs['origin'] = kwargs.get('origin', 'lower')
        pmn,pmx = perc
        mn = None
        mx = None
        havenorm = 'norm' in kwargs
        hs = []
        for ax,a in zip(self.axes_all, arrs):
            if mask is not None:
            ...
            if not havenorm:
                if scale == 'log':
                    ...
                    if pmx is not None:
                        pos = a[a>0]
                        if pos.size:
                            mx = np.percentile(pos, pmx)
                        else:
                            mx = 1

                    kwargs['norm'] = mcolors.LogNorm(mn, mx)
                else:
                    if pmn is not None:
                        mn = np.percentile(np.ma.compressed(a), pmn)
                        kwargs['vmin'] = mn
                    if pmx is not None:
                        mx = np.percentile(np.ma.compressed(a), pmx)
                        kwargs['vmax'] = mx
            res = ax.imshow(a, **kwargs)
            hs.append(res)
    ...
    def plot(self, *args, **kwargs):
        if len(args) > 1 and cbook.is_string_like(args[-1]):
            c = (args.pop(),)
        else:
            c = ()
        if len(args) == 2:
        ...
        lhs = []
        for ax,y in zip(self.axes_all, ys):
            args = x + (y,) + c
    ...
    def titles(self, titles, **kwargs):
        ths = []
        for ax,t in zip(self.axes_all, titles):
            res = ax.set_title(t, **kwargs)
            ths.append(res)
...
class ComplexSize(Size.Add):
    def __init__(self, z):
        self._a = Size.Scaled(getattr(z, 'real', 0))
        self._b = Size.Fixed(getattr(z, 'imag', z))

    @classmethod
    def from_any(cls, size, fraction_ref=None):
...
def size2str(s):
    try:
        ...
        a,b = s._fraction, s._fraction_ref
        tp = 'F'
    return tp + '(' + _size2str(a) + ',' + _size2str(b) + ')'
    ...
    def __init__(self, fig,
        ...
        *rect=[left, bottom, width, height]* (in
        ...
        Optional keyword arguments:
          ...
          add_all           True      [ True | False ]
          share_all         False     [ True | False ]
          share_x           True      [ True | False | "all" ]
          share_y           True      [ True | False | "all" ]
          aspect            True      [ True | False ]
          label_mode        "L"       [ "L" | "1" | "all" | "y" ]
          cbar_mode         None      [ "each" | "single" | "edge" ]
          cbar_location     "right"   [ "left" | "right" | "bottom" | "top" ]
          cbar_pad          None
          cbar_size         "5%"
        ...
        *cbar_set_cax* : if True, each axes in the grid has a cax
          attribute that is bind to associated cbar_axes.
        """
        self._allwidths = allwidths
        self._allheights = allheights
        self._nrows = len(allheights)//2
        self._ncols = len(allwidths)//2
        self._debug = debug
        ...
        self._axes_pad = allwidths[1]

        self._colorbar_mode = cbar_mode
        self._colorbar_location = cbar_location
        if cbar_pad is None:
            self._colorbar_pad = self._axes_pad
        else:
            self._colorbar_pad = cbar_pad

        self._colorbar_size = cbar_size

        self._init_axes_pad(self._axes_pad)
        ...
        if label_mode == 'y':
            if share_x is None: share_x = 'all'
            if share_y is None: share_y = False
            share_all = False
#        else:
#            share_all = False
#            share_x = True
#            share_y = True
        ...
        for i in range(self.ngrids):
            ...
            if share_all:
                ...
                if sharex is None:
                    if share_x == "all":
                        self._column_refax = self._ncols*[ax]
                    else:
                        self._column_refax[col] = ax
                if sharey is None:
                    if share_y == "all":
                        self._row_refax = self._nrows*[ax]
                    else:
                        self._row_refax[row] = ax
    ...
    def _update_locators(self):
        ...
        allwidths = list(self._allwidths)
        allheights = list(self._allheights)

#        for col,ax in enumerate(self._column_refax):
#            h.append(ComplexSize(allwidths.pop(0)))
#            sz = ComplexSize(allwidths.pop(0))
#            h_ax_pos.append(len(h))
#            h.append(sz)
        for i in range(len(allwidths)//2):
            h.append(ComplexSize(allwidths.pop(0)))
            sz = ComplexSize(allwidths.pop(0))
            h_ax_pos.append(len(h))
            h.append(sz)
            if (self._colorbar_mode == "each" or
                    (self._colorbar_mode == 'edge' and
                        col == self._ncols - 1)) and self._colorbar_location == "right":
                h.append(ComplexSize.from_any(self._colorbar_pad, sz))
                h_cb_pos.append(len(h))
                h.append(ComplexSize.from_any(self._colorbar_size, sz))

        h.append(ComplexSize(allwidths.pop(0)))
        assert len(allwidths) == 0

        if self._debug: sys.stderr.write('h '+str(map(size2str, h)))


        v_ax_pos = []
        v_cb_pos = []
#        for row,ax in enumerate(self._row_refax[::-1]):
        for i in range(len(allheights)//2):
            v.append(ComplexSize(allheights.pop(0)))
            sz = ComplexSize(allheights.pop(0))
            v_ax_pos.append(len(v))
            v.append(sz)

        v.append(ComplexSize(allheights.pop(0)))
        assert len(allheights) == 0
        ...
        if self._colorbar_mode == "single":
            if self._colorbar_location == "right":
                ...
                #sz = Size.Fraction(Size.AxesY(self.axes_llc), self._ncols)
                sz = Size.Fraction(self._ncols, Size.AxesY(self.axes_llc))
                v.append(ComplexSize.from_any(self._colorbar_pad, sz))
                v.append(ComplexSize.from_any(self._colorbar_size, sz))
                locator = self._divider.new_locator(nx=0, nx1=-1, ny=-2)
            if self._colorbar_location in ("right", "top"):
                for i in range(self.ngrids):
                    self.cbar_axes[i].set_visible(False)
...
def gapgridargs(widths, heights, wgap=.5j, hgap=.5j, left=.5j, right=.5j, top=.5j, bottom=.5j,
                fig=None, rect=[0,0,1,1], **kwargs):
    try:
        int(widths)
    except TypeError:
        pass
    ...
    cbar_mode = kwargs.get('cbar_mode', None)

    allwidths = [left]
    for w in widths:
        allwidths.extend([w, wgap])
    allwidths[-1] = right

    allheights = [bottom]
    for h in heights:
        allheights.extend([h, hgap])
    allheights[-1] = top
    ...
    return (fig, rect, allwidths, allheights), kwargs


def gapgrid(widths, heights, wgap=.5j, hgap=.5j, left=.5j, right=.5j, top=.5j, bottom=.5j,
            fig=None, rect=[0,0,1,1], clf=False, **kwargs):
    args,kwargs = gapgridargs(widths, heights, wgap, hgap, left, right, top, bottom,
                              fig, rect, **kwargs)
    if clf:
        args[0].clf()
    return GapGrid(*args, **kwargs)


def gapaxes(left=.96j, right=.8j, top=.6j, bottom=.6j,
            fig=None, rect=[0,0,1,1], cb=None, **kwargs):
    widths = [1.]
    heights = [1.]
    if cb is not None:
        if cb is True:
        ...
        try:
            ...
            wid = cb
            kwargs.setdefault('wgap', wid)
        else:
            kwargs['wgap'] = pad
        widths.append(wid)
        kwargs.setdefault('share_y', False)
            
    grid = gapgrid(widths, heights, left=left, right=right, top=top, bottom=bottom,
                   fig=fig, rect=rect, **kwargs)
    ax = grid.axes_all[0]
    ax._grid = grid
    plt.sca(ax)
    if cb is not None:
